let E=[],c=[{id:"classic",name:"Original Portfolio",preview:"Aa"},{id:"gears",name:"Share Tech Mono",preview:"Aa"}],q="Custom";const A="#a51d24",p="classic",C="portfolio-accent-color",S="portfolio-font-theme";function m(o){document.documentElement.style.setProperty("--color-accent",o)}function h(){try{return localStorage.getItem(C)||A}catch{return A}}function b(o){try{localStorage.setItem(C,o)}catch{}}function g(o){document.documentElement.dataset.fontTheme=o==="gears"?"gears":p}function k(){try{return localStorage.getItem(S)==="gears"?"gears":p}catch{return p}}function B(o){try{localStorage.setItem(S,o)}catch{}}async function T(){const o=document.getElementById("picker-trigger"),s=document.getElementById("picker-panel"),O=document.getElementById("panel-close"),r=document.getElementById("slot-grid"),f=document.getElementById("equipped-name"),l=document.getElementById("font-grid"),y=document.getElementById("font-equipped-name");if(!o||!s||!r||!f||!l||!y)return;try{const t=await(await fetch("/data/ui-text.json")).json();E=t.accentPicker?.colors||[],q=t.accentPicker?.customName||"Custom",c=t.accentPicker?.fontPicker?.options||c}catch{}const d=h(),u=k();m(d),g(u),r.innerHTML=E.map(e=>`
      <button
